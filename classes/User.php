...
class User {
...

	{
		...
		elseif (($user) && (password_verify($password, $user['password'])) && ($user['locked'] == 1)) {
			$pin = $user['supportpin'];
        		return "Account is locked. Contact support for more information. $pin";
}

 		elseif (($user) && (password_verify($password, $user['password'])) && ($user['locked'] == 0) && ($user['authused'] == 1 && ($oneCode == $_POST['auth'])))  {
			return $user;
...

	{
...

		{
...

			{
                ...
                $insert_date = date("n/j/Y g:i a");
                $pin = rand(10000,99999);
                $query = $this->mysqli->query("INSERT INTO users (`date`, `ip`, `username`, `email`, `password`, `supportpin`) VALUES ('$insert_date', '$ip', '$username', '$email', '$password', '$pin');");
                $query = $this->mysqli->query("INSERT INTO logins (`user`, `datetime`, `user_agent`, `ip`, `success`) VALUES ('$username', '$insert_date', '$agent', '$ip', 1);");
